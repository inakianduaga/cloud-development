...
docker run -d -p 80:80 -p 443:443 -v $certs_path:/etc/nginx/certs --env-file $USERS_PATH --env-file $CONFIG_PATH -e PROXY_HOST=$(getDockerBridgeIp) --name $cloud_frontend_proxy $cloud_frontend_proxy
